...
\begin{itemize}
      \item Added attribute \Rfunction{dose.units} to dose grid in class \Rfunction{RTdata} to enable specification and processing of dose units.
      \item New class \Rfunction{zDVH} (along with relevant methods) to store one or more axially-segmented and calculated dose-volume histograms. The class otherwise contains all elements of the \Rfunction{DVH} class and is largely interchangeable with other \Rfunction{DVH} objects.
      \item Added \Rfunction{janedoe.RTdata} and \Rfunction{stomach} to package data contents in order to provide an example of a \Rfunction{RTdata} and \Rfunction{zDVH} object, respectively.
      \item New plot functionality for axial dose-volume histogram displayed from one or more \Rfunction{zDVH} objects.
      \item New method for \Rfunction{compareStructures()} ("DSC") which calculates Dice similarity coefficients between pairs of structures.
\end{itemize}
...
\label{DVH.import}
The \Rfunction{read.DVH()} function is designed to take an input text file and output a list of DVH data objects containing all relevant data. Supported file types currently include Varian's Aria/Eclipse platform (v.10 and v.11), CadPlan (for archival/historical data), and TomoTherapy. Other treatment planning systems are not currently supported however will be included in future releases.
...
For nested DVH list data generated using \Rfunction{read.DVH()} on multiple input files, the \Rfunction{getStructureList()} function provides access to individual structures.  Using \Rfunction{getStructureList()}, desired instances of a given structure are selected from among a list of \Rfunction{DVH.list} objects).
<<echo=TRUE,results=verbatim>>=
...
@
\end{center}
\caption{Axially-segmented DVH (zDVH) displayed as a differential DVH in a three dimensional projection. Plotting in this format simply requires input of a single zDVH object.}
\label{fig:seven}
\end{figure}
...
Many thanks to Brandon Whitcher for his phenomenal and generous support regarding the \Rpackage{oro.dicom} pacakge, and to Daniel Wollschlaeger for his insights and contributions regarding DVH file formatting, dose unit interconversion, and inclusion of CadPlan data. Thank you to Lukasz Matulewicz for his contributions regarding inclusion of TomoTherapy-formatted data.
...
\item Last release (v1.0.8) on 2014-05-31
\item Current release (v1.0.9) on 2014-11-21
\end{itemize}
