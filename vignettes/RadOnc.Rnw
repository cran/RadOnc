...
\begin{itemize}
\item New function \Rfunction{gEUD()} to calculate generalized equivalent uniform dose values from \Rfunction{DVH} and \Rfunction{DVH.list} objects.
\item New function \Rfunction{LQE()} to perform linear quadratic extrapolation for iso-effective dose conversion among \texttt{DVH} and \texttt{DVH.list} objects with different fractionation.
\item New method for \Rfunction{sum()} to create DVH sum from two or more non-overlapping \Rfunction{DVH} and/or \Rfunction{DVH.list} object(s).
\item Extended \Rfunction{read.DVH()} function to import multiple files simultaneously (if desired) and expanded import formats to include CadPlan files (for archival/historical data).
\item Expanded functionality of DVH parameter access; now a user may specify output type in parentheses (e.g. \texttt{DVH["V20Gy(cc)"]}).
...
\begin{figure}[H]
\begin{center}
<<label=fig9,fig=FALSE,echo=TRUE,eval=FALSE>>=
plot(mandible)
@
...
@
<<label=fig11,fig=TRUE,echo=FALSE,results=hide,height=10,width=2>>=
...
\label{previous}
\begin{itemize}
\item Last release (v1.0.2) on 2013-11-25
