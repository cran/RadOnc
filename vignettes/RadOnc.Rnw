...
\begin{itemize}
      \item Functionality expanded for \Rfunction{[]} accessor method to \Rfunction{DVH.list} and \Rfunction{structure.list} objects to enable pattern matching (e.g. \Rfunction{janedoe["KIDNEY\$"]} and handling of redundant structure names in \Rfunction{DVH.list}. 
      \item New methods for \Rfunction{as()} function to convert a list of \Rfunction{DVH} and/or \Rfunction{DVH.list} objects to a single combined \Rfunction{DVH.list} object and a list of \Rfunction{structure3D} and/or \Rfunction{structure.list} objects to a single combined \Rfunction{structure.list} object.
      \item Expanded \Rfunction{read.DICOM.RT()} functionality to include calculation of DVHs from dose grid information. DVH calculations can be enabled by specifying logical parameter \Rfunction{DVH=TRUE} (default). Also added \Rfunction{calculate.DVH()} function to perform DVH calculation given one or more input structure set(s) and dose grid data. This function now also implicitly extends \Rfunction{read.DVH(..., method="dicom")} to enable DVH calculation from DICOM-RT data.
      \item \Rfunction{LQE()} now supports fractionation specification in either dose per fraction or total number of fractions (parameters \Rfunction{N} and \Rfunction{dose.units} were introduced for this purpose).
      \item Added \Rfunction{janedoe.RTdata} to package data contents in order to provide an example of a \Rfunction{RTdata} object.
      \item \Rfunction{DVH} class expanded to include patient identifying information (new parameters \Rfunction{patient} and \Rfunction{ID})
\end{itemize}
...
\begin{figure}[H]
\begin{center}
<<label=fig10,fig=FALSE,echo=TRUE,eval=FALSE>>=
plot(mandible)
@
...
@
<<label=fig12,fig=TRUE,echo=FALSE,results=hide,height=10,width=2>>=
...
Many thanks to Brandon Whitcher for his phenomenal and generous support regarding the \Rpackage{oro.dicom} pacakge, and to Daniel Wollschlaeger for his insights and contributions regarding DVH file formatting, dose unit interconversion, and inclusion of CadPlan data.
...
\item Last release (v1.0.5) on 2014-02-19
\end{itemize}
