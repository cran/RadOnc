...
\label{DVH.import}
The \Rfunction{read.DVH()} function is designed to take an input text file and output a list of DVH data objects containing all relevant data. Supported file types currently include Varian's Aria/Eclipse platform (v.10 and v.11), CadPlan (for archival/historical data), TomoTherapy, and Elekta's Monaco platform. Other treatment planning systems are not currently supported however will be included in future releases.
...
\item Last release (v1.1.0) on 2015-04-21
\end{itemize}
