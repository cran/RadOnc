...
\begin{itemize}
\item New function \Rfunction{read.DICOM.RT()} to import 3-dimensional structural information from one or more DICOM-RT files.
\item New class \Rfunction{structure3D} to store 3-dimensional information encoding a structure.
\item New functions to support interaction with \Rfunction{structure3D} class.
\item New class \Rfunction{structure.list} to store a list of \Rfunction{structure3D} objects.
\item New functions to support interaction with \Rfunction{structure.list} class.
\item New function \Rfunction{compareStructures()} to assess similarities and differences among two or more \Rfunction{structure3D} objects within a \Rfunction{structure.list}.
\end{itemize}
...
@
Note that specific parameter keywords include: \Rfunction{"Dmax"} (maximum dose), \Rfunction{"Dmin"} (minimum dose), \Rfunction{"Dmean"} (mean dose), \Rfunction{"Dintegral"} (estimated integral dose), \Rfunction{"DRx"} (prescription dose), and \Rfunction{"volume"} (total structure volume). If an improper parameter is specified however, \Rfunction{NA} results will be returned for the affected parameter(s):
<<echo=TRUE,results=verbatim>>=
...
\begin{figure}[H]
\begin{center}
<<label=fig8,fig=FALSE,echo=TRUE,eval=FALSE>>=
plot(mandible)
@
...
@
<<label=fig10,fig=TRUE,echo=FALSE,results=hide,height=10,width=2>>=
...
\label{previous}
\begin{itemize}
\item Last release (v1.0.1) on 2013-10-23
